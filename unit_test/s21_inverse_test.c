...
START_TEST(s21_inverse_1) {
  ...
  matrix_t B;
  int out = s21_inverse_matrix(&A, &B);
  int res = s21_eq_matrix(&B, &C);
  ck_assert_int_eq(out, OK);
...
START_TEST(test_one_by_one) {
  ...
  if (codec == OK) {
    ...
    ck_assert_int_eq(result.matrix[0][0] == (1.0 / m.matrix[0][0]), OK);
...
START_TEST(determinant) {
  const int size = 2;
  matrix_t m = {0};
  int code = s21_create_matrix(size, size, &m);
  if (code == OK) {
    m.matrix[0][0] = 1;
    m.matrix[0][1] = 1;
    m.matrix[1][0] = 1;
...
START_TEST(inverse_matrix_1) {
  ...
  int row = 3;
  int col = 3;
  int code = s21_create_matrix(row, col, &matr1);
  if (code == OK) {
...
Suite *suite_inverse_matrix(void) {
  Suite *s = suite_create("\033[10;35;40m INVERSE MATRIX \033[0;0;0m\n");
